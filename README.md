...
- **Backend** — FastAPI + WebSocket. Polls each tmux pane (`tmux capture-pane`) ~every 700 ms, captures 200 lines of scrollback by default, runs detectors, broadcasts state diffs. Sends keystrokes back via `tmux send-keys -l` (literal, shell-safe). User-supplied detector regexes run with a hard timeout, so a bad pattern can't wedge the loop.
...
capture, discovery, starred panes, push, usage tracking, and detector patterns.
...
the exec'd `usage.command` stay YAML-only.
